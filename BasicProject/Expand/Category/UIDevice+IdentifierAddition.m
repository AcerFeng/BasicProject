...
- (NSString *)LF_uuid {
    ...
    if (uuid.length == 0) {
        [[NSUserDefaults standardUserDefaults] setObject:[self LF_createUUID] forKey:key];
...
- (NSString *)LF_udid {
    NSString *udid = [[LFUDIDGenerator sharedInstance] UDID];
    if (udid.length == 0) {
        udid =[self LF_uuid];
        [[LFUDIDGenerator sharedInstance] saveUDID:udid];
    }
    return udid;
}
...
- (NSString *)LF_macaddressMD5 {
    ...
    if (macid.length == 0) {
        NSString *macaddress = [[UIDevice currentDevice] LF_macaddress];
